...
public class OrderDTO {
	...
	private List<String> listdetalleproductos = new ArrayList<String>();
	List<DTO> listadoDTO = new ArrayList<DTO>();
		...
		for (Order1 order1 : orderList) {
			...
			for (OrderDetail orderDetail : order1.getOrderDetailList()) {
				detalleproductos= detalleproductos  + orderDetail.getQuantity() +" x "+ orderDetail.getProductDescription()+"<br>";
